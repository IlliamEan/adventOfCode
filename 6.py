...
aoeu="""
"""
inn = """toggle 461,550 through 564,900
...
def op2 (m):
    if m.group(1) == "turn on":
        ...
        l = lambda a: a - 1 if a > 0 else 0;
    else:
        l = lambda a: a + 2;
    return l

def turning (operation):
    for inst in inn:
        m = re.match(r"(turn on|turn off|toggle) ([0-9]*),([0-9]*) through ([0-9]*),([0-9]*)", inst)
        
        l = operation(m)
            
        rect(int(m.group(2)), int(m.group(3)), int(m.group(4)), int(m.group(5)), l)
